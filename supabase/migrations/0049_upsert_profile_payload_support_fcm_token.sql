-- Ensure upsert_profile_payload can persist FCM token.
-- Without this, the client can call updateUserProfile({ fcmToken }) successfully,
-- but `public.profiles.fcm_token` remains NULL because the RPC never updates it.
...
begin
  ...
  set
    ...
    fcm_token = case when p_fields ? 'fcm_token' then nullif(trim(p_fields->>'fcm_token'), '') else p.fcm_token end
