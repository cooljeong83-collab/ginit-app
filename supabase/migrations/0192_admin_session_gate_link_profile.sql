-- Admin login: link profiles.auth_user_id from Google email when missing, then gate.
...
begin
  ...
  if v_row.admin is distinct from 'y' then
    return jsonb_build_object(
      ...
      'reason', 'not_admin',
      'hint', format(
        'UPDATE profiles SET admin = ''y'' WHERE id = %L; (또는 auth.users.email과 일치하는 app_user_id 행)',
        v_row.id
      ),
