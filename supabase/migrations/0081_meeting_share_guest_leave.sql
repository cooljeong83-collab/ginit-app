-- 웹 게스트: 모임 나가기 — participantIds / joinRequests 에서 제거하고, 참여자 투표 집계·participantVoteLog 에서 해당 게스트 반영 제거.
...
begin
  ...
  if v_in_part then
    ...
    into v_part_new
    from jsonb_array_elements_text(coalesce(v_fs->'participantIds', '[]'::jsonb)) as t(x)
