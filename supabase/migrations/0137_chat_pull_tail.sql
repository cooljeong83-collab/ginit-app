-- 최신 메시지 N건 (seq 내림차순). 채팅 UI 초기/Realtime 리프레시용.

create or replace function public.chat_pull_tail(
  p_me text,
  p_room_kind text,
  p_room_id text,
...
declare
  ...
  v_mid uuid;
  v_canonical text;
  v_lim int := greatest(1, least(coalesce(p_limit, 20), 200));
begin
  if v_me is null or v_rid is null then
    return jsonb_build_object('rows', '[]'::jsonb, 'max_seq', null, 'min_seq', null, 'has_more', false, 'error', 'invalid_args');
  end if;
  if v_kind not in ('meeting', 'social_dm') then
    return jsonb_build_object('rows', '[]'::jsonb, 'max_seq', null, 'min_seq', null, 'has_more', false, 'error', 'invalid_room_kind');
  end if;
  ...
  return (
    ...
    agg as (
      select
        ...
        (select max(seq) from capped) as max_seq,
        (select min(seq) from capped) as min_seq,
        (select count(*) > v_lim from numbered) as has_more
    )
    select jsonb_build_object(
      'rows', (select rows from agg),
      'max_seq', (select max_seq from agg),
      'min_seq', (select min_seq from agg),
      'has_more', (select has_more from agg),
...
revoke all on function public.chat_pull_tail(text, text, text, int) from public;
grant execute on function public.chat_pull_tail(text, text, text, int) to anon, authenticated;
