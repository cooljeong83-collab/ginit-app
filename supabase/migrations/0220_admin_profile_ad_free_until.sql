-- admin_update_profile: profiles.ad_free_until (광고 비노출 만료 시각)
...
begin
  ...
  set
    ...
    ad_free_until = case
      when not (v_fields ? 'ad_free_until') then p.ad_free_until
      when v_fields->'ad_free_until' is null or nullif(trim(v_fields->>'ad_free_until'), '') is null then null
      else (v_fields->>'ad_free_until')::timestamptz
    end
