-- 피드 후기 카드: 해당 모임(meeting_id)의 코멘트 전체를 작성 시각 오름차순으로 comments[]에 담음

create or replace function public.feed_meeting_review_comments_for_meeting(p_meeting_id uuid)
returns jsonb
language sql
stable
security definer
...
as $$
  select coalesce(
    jsonb_agg(btrim(mr.comment) order by mr.created_at asc, mr.id asc),
    '[]'::jsonb
  )
...
revoke all on function public.feed_meeting_review_comments_for_meeting(uuid) from public;
grant execute on function public.feed_meeting_review_comments_for_meeting(uuid) to anon, authenticated;
  ...
  from (
    with review_rows as (
      select
        ...
        public.feed_meeting_review_comments_for_meeting(m.id) as comments,
...
comment on function public.list_feed_meeting_reviews_for_region(text, int) is
  '탐색 피드 후기 캐러셀. comments[] = 해당 모임 전체 코멘트(작성 시각 오름차순).';
