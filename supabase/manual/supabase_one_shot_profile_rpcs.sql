-- =============================================================================
-- 한방 보정 SQL — Supabase 대시보드 → SQL Editor → 전체 복사 후 Run 한 번
--
-- 해결하는 대표 오류:
--   column p.signup_provider does not exist
--   column p.ranking_points does not exist
--   could not find the function public.ensure_profile_minimal / upsert_profile_payload /
--     get_profile_public_by_app_user_id in the schema cache
--
-- 전제: public.profiles 테이블이 이미 존재함 (hybrid_init 0001 등으로 생성된 상태).
--      profiles 자체가 없으면 이 스크립트보다 먼저 0001_hybrid_init.sql 을 적용해야 합니다.
-- =============================================================================

-- ─── 1) profiles 에 누락되기 쉬운 컬럼 (여러 마이그레이션에 흩어져 있던 것) ───
...
-- g_trust 범위 (0002 와 동일). 기존 데이터가 0~100 을 벗어나면 이 구문은 실패할 수 있음.
alter table public.profiles drop constraint if exists profiles_g_trust_range;
alter table public.profiles
  add constraint profiles_g_trust_range check (g_trust >= 0 and g_trust <= 100);

-- ensure_profile_minimal 의 ON CONFLICT (app_user_id) 는 app_user_id 에 UNIQUE 가 있어야 합니다.
...
select
  exists(select 1 from information_schema.columns where table_schema = 'public' and table_name = 'profiles' and column_name = 'signup_provider') as col_signup_provider_ok,
  exists(select 1 from information_schema.columns where table_schema = 'public' and table_name = 'profiles' and column_name = 'ranking_points') as col_ranking_points_ok,
  exists(select 1 from information_schema.columns where table_schema = 'public' and table_name = 'profiles' and column_name = 'penalty_count') as col_penalty_count_ok;
...
) as fn_ensure_profile_minimal_ok,
exists(
  select 1 from pg_proc p
  join pg_namespace n on n.oid = p.pronamespace
